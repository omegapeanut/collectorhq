// CollectorHQ — shared card catalog with price history + recent sales
// Prices/names modeled on real market listings (Jul 2026).
...
function genHistory(seed, start, end, deltaPct, n = 52) {
  const r = rng(seed);
  const pts = [];
  for (let i = 0; i < n; i++) {
    const t = i / (n - 1);
    const base = start * Math.pow(end / start, t);
    const noise = 1 + (r() - 0.5) * 0.10 * (1 - t * 0.45);
    pts.push(base * noise);
  }
  pts[n - 1] = end;
  // anchor last-week point so 7-day delta matches the quoted movement
...
export const CARDS = {
  ...
  charmander:{ seed: 17, art: null, game: 'POKÉMON', name: 'Charmander', set: 'Mega Evolution Promos', number: 'Promo · 038', rarity: 'Promo', finish: 'Holofoil', cond: 'RAW NM', pop: 'PSA 10 pop 894', price: 40.40, start: 62, deltaPct: -10.60 },
  ...
  gengar:    { seed: 20, art: null, game: 'POKÉMON', name: 'Gengar (Cosmos Holo) (EB Games Exclusive)', set: 'Miscellaneous Cards & Products', number: '050/088', rarity: 'Rare', finish: 'Holofoil', cond: 'RAW NM', pop: 'PSA 10 pop 402', price: 125.73, start: 88, deltaPct: 4.40 },
...
export function getCard(id) {
  ...
  const chips = GAME_CHIPS[c.game] || GAME_CHIPS['MAGIC'];
  return {
    id,
    ...c,
    ...chips,
